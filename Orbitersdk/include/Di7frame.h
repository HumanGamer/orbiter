...
int SDL_GetNumGamepads() { return 0; }
	int joy_count = 0;
public:
	CDIFramework7() { SDL_joy_count = SDL_GetNumGamepads(); }
	...
	HRESULT CreateJoyDevice(HWND,	DWORD idx = 0) { 
		joy_count = SDL_GetNumGamepads();
		if (idx < joy_count) sdl_ctrl = (SDL_GameController*)SDL_OpenGamepad(idx);
		return S_OK; 
	}
	void DestroyJoyDevice() { if(sdl_ctrl){SDL_CloseGamepad((SDL_Gamepad*)sdl_ctrl); sdl_ctrl=nullptr;} }
